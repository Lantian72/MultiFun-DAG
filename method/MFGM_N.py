...
def single_test(N, T, K, P, L, sigma=None, s0=None, h=None, E_true=None, W_true=None):
    ...
    # scale = [1, 0.5]
    if h is None:
        E_true, W_true, g, h, true_a = sim_setup.generate_DAG(N=N, T=T, K=K, P=P, L=L, s0=14, sigma=sigma)
    ...
    CK, CL = notears_multifunctional(true_a, L, lambda1=0.01, c_threshold=0, h_tol=1e-8, rho_max=1e+16)
    C = np.zeros((sum(L) * K, sum(L) * K))
    ...
    for i in range(P):
        for j in range(P):
            ...
            E_est[i, j] = np.sum(abs(CK_ij)) * np.sum(abs(CL_ij)) / L[i] / L[j] / K
    E_est[np.abs(E_est) < 0.3] = 0
    np.savetxt('MFGM-N200.csv', C_est)
    node = {}
    for p in range(P):
        for l in range(L[p]):
            node[l + sum(L[:p])] = p
