...
def single_test(N, T, K, P, L, sigma=None, s0=None, h=None, E_true=None, W_true=None):
    if h is None:
        E_true, W_true, g, h, true_a = sim_setup.generate_DAG(N=N, T=T, K=K, P=P, L=L, s0=s0, sigma=sigma, seed=233)
    ...
    for p in range(P):
        for l in range(L[p] * K):
            node[l + sum(L[:p]) * K] = p
    WW = np.zeros((K * sum(L), K * sum(L)))
    for p in range(P):
        for _p in range(P):
            WW[sum(L[:p]) * K:sum(L[:p + 1]) * K, sum(L[:_p]) * K:sum(L[:_p + 1]) * K] \
                = np.kron(np.full((L[p], L[_p]), W_true[p, _p]), np.identity(K))
    # print(varsortability(a.reshape(N, sum(L) * K), WW))
    E_est = notears_separate(X=true_a.reshape(N, sum(L) * K), lambda1=0.01, node=node)
    np.savetxt('NS-C200.csv', E_est)
    return 0
    ...
    Y_est = est_function(a.reshape(N, sum(L) * K) @ E_est, N, L, K, T, v, node)
...
if __name__ == '__main__':
    P = 10
    L = np.full(P, 2)
    single_test(N=200, T=100, K=2, P=P, s0=14, L=L, sigma=0.1)